...
def concolic_test(testfunc, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    index = 0
    for (child, parent) in zip(cur_path_constr, cur_path_constr_callers):
        index += 1
  ...
  ##
    ...
    ##
        current_constraint = sym_and(sym_not(child), *cur_path_constr[:index-1])  
